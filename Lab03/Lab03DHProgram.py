...
class SecurePRNG:
    ...
    def __init__(self, seed_int):
        ...
        seed_bytes = seed_int.to_bytes(math.ceil(seed_int.bit_length() / 8), "big")
...
def xor_crypt(data, prng):
    ...
    for i in range(len(data)):
        result += bytes([data[i] ^ keystream[i]])
    ...
    def intercept(self, sender, recipient, payload):
        ...
        if isinstance(payload, bytes):
            ...
            if sender == "Alice":
                ...
                print("Mallory viewing Alice's Message: ", plaintext)
                modified_plaintext = plaintext.replace(b"park", b"hotel")

                return xor_crypt(modified_plaintext, self.bob_prng)
            
            elif sender == "Bob":
                plaintext = xor_crypt(payload, self.bob_prng)
                print("Mallory viewing Bob's Message: ", plaintext)
                modified_plaintext = plaintext.replace(b"park", b"hotel")

                return xor_crypt(modified_plaintext, self.alice_prng)
...
def main():
    ...
    message = b"<Hello, meet me at the park at 10am for the snowboard>" # Put in your test message here
    ...
    message = b"Hello, meet me at the park at 10am for the snowboard."
    encrypted_msg = xor_crypt(message, alice.session_prng)
    delivered_data = net.send("Alice", "Bob", encrypted_msg)
